...
class CircuitBreaker:
    ...
    def allow_request(self) -> bool:
        ...
        CLOSED / HALF_OPEN always allow (HALF_OPEN lets a single probe through).
        OPEN fails fast until ``reset_timeout_seconds`` has elapsed, at which
        point we optimistically move to HALF_OPEN and allow one probe.
        """
    ...
    def record_success(self) -> None:
        ...
        A success clears the failure streak. In HALF_OPEN, enough consecutive
        probe successes close the circuit again.
        """
    ...
    def record_failure(self) -> None:
        ...
        A HALF_OPEN probe failure re-opens immediately (distinct reason from a
        CLOSED-state threshold breach). The two triggers stay in separate
        branches so the transition log records why the circuit opened.
        """
