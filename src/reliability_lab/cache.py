...
class ResponseCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        # 2. Evict expired entries lazily on read.
        now = time.time()
        self._entries = [e for e in self._entries if now - e.created_at <= self.ttl_seconds]

        # 3. Find the most similar surviving entry.
        best_score = 0.0
        ...
        # 4. Only serve above threshold, and only if it is not a false hit.
    ...
    def _tokenize(text: str) -> list[str]:
        """Word tokens + character 3-grams, e.g. "hello" → hello, hel, ell, llo."""
        tokens: list[str] = []
    ...
    def similarity(a: str, b: str) -> float:
        ...
        Bag-of-features cosine keeps far more signal than Jaccard token overlap:
        near-identical phrases score high while differing in a single word, and
        unrelated phrases collapse toward zero.
        """
        if a == b:
            return 1.0
        va = Counter(ResponseCache._tokenize(a))
        vb = Counter(ResponseCache._tokenize(b))
        if not va or not vb:
            return 0.0
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        """Look up a cached response from Redis (exact key, then similarity scan)."""
        if _is_uncacheable(query):
            return None, 0.0

        # Fast path: deterministic exact-match key.
        exact_key = f"{self.prefix}{self._query_hash(query)}"
        exact_response = self._redis.hget(exact_key, "response")
        if exact_response is not None:
            return exact_response, 1.0

        # Slow path: scan the namespace and score each stored query.
        best_score = 0.0
    ...
    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        """Store a response in Redis with an automatic TTL."""
        if _is_uncacheable(query):
            return
