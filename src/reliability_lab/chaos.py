...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    """Derive average recovery time (open → closed) from breaker transition logs.

    Each ``open`` transition is paired with the next ``closed`` transition on the
    same breaker; the gap (in ms) is one recovery. Returns the mean across all
    such pairs, or None if no circuit ever recovered.
    """
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    """Run a single named chaos scenario and collect reliability metrics."""
    ...
    for _ in range(config.load_test.requests):
        ...
        if result.route == "fallback":
            ...
            # "primary" or "cache_hit:*" — both served a real answer.
            metrics.successful_requests += 1
...
def scenario_passed(name: str, result: RunMetrics) -> bool:
    ...
    if name == "primary_timeout_100":
        # Primary is dead → traffic must survive via the backup fallback.
        return result.availability >= 0.85 and result.fallback_success_rate >= 0.85
    if name == "all_healthy":
        return result.availability >= 0.9
    # Flaky / default scenarios: system should still mostly succeed.
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        combined.scenarios[scenario.name] = (
            "pass" if scenario_passed(scenario.name, result) else "fail"
        )
